...
def factorize_weight(generator,affine_weight_dct, layer_idx='all'):
    ...
    if gan_type == 'pggan':
        ...
        if layer_idx == 'all':
            ...
            layers = parse_indices(layer_idx,
                                   min_val=0,
                                   max_val=generator.num_layers - 2)

    # Factorize semantics from weight.
    weights = []
    for idx in layers:
        weight = affine_weight_dct[idx]
        weights.append(weight.cpu().detach().numpy().transpose())
